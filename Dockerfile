#Pull from specific centos 7 version
FROM centos:centos7.6.1810
LABEL maintainer=chad.smykay@hpe.com
# Runtime enviroment variables.
ARG BUILD_DATE=05-07-2020
ARG BUILD_VERSION=10.0
#Labels
LABEL   org.label-schema.build-date=$BUILD_DATE \
        org.label-schema.name="ess-hpecp-mofed-customer-image:latest" \
        org.label-schema.description="Mellanox drive enabled docker image for a customer's installed packages" \
        org.label-schema.url="https://github.com/" \
        org.label-schema.vcs-url="https://github.com/csmykay/ess-hpecp-mofed-customer-image" \
        org.label-schema.vendor="HPE" \
        org.label-schema.version=$BUILD_VERSION \
        org.label-schema.docker.cmd="docker run -v ess-hpecp-mofed-customer-image:latest -d ess-hpecp-mofed-customer-image:latest"
#Here if you need a user to do user/group work
#groupadd group &&
#useradd -g user -s /bin/bash user &&
#echo -e "$5$VZ3KNRVQS3g$NPM9hwldIY5qL27oL3y/VVCwPjKeBFaxBiUeYE2pWi3\n$5$VZ3KNRVQS3g$NPM9hwldIY5qL27oL3y/VVCwPjKeBFaxBiUeYE2pWi3" | passwd user &&     
#Install EPEL repos.
#RUN yum install -y epel-release
COPY /etc/yum.repos.d/* /etc/yum.repos.d
RUN  yum clean all
RUN rm -rf /var/cache/yum/*
#RUN yum repolist --disablerepo=extras --disablerepo=updates
RUN yum makecache
#Install base packages for customer site
#Install add-on packages install for customer site
RUN yum install -y abrt-2.1.11-52.el7.centos.x86_64 \
abrt-addon-ccpp-2.1.11-52.el7.centos.x86_64 \
abrt-addon-kerneloops-2.1.11-52.el7.centos.x86_64 \
abrt-addon-pstoreoops-2.1.11-52.el7.centos.x86_64 \
abrt-addon-python-2.1.11-52.el7.centos.x86_64 \
abrt-addon-vmcore-2.1.11-52.el7.centos.x86_64 \
abrt-addon-xorg-2.1.11-52.el7.centos.x86_64 \
abrt-cli-2.1.11-52.el7.centos.x86_64 \
abrt-console-notification-2.1.11-52.el7.centos.x86_64 \
abrt-dbus-2.1.11-52.el7.centos.x86_64 \
abrt-libs-2.1.11-52.el7.centos.x86_64 \
abrt-python-2.1.11-52.el7.centos.x86_64 \
abrt-retrace-client-2.1.11-52.el7.centos.x86_64 \
abrt-tui-2.1.11-52.el7.centos.x86_64 \
acl-2.2.51-14.el7.x86_64 \
aic94xx-firmware-30-6.el7.noarch \
alsa-firmware-1.0.28-2.el7.noarch \
alsa-lib-1.1.6-2.el7.x86_64 \
alsa-tools-firmware-1.1.0-1.el7.x86_64 \
apr-1.4.8-3.el7_4.1.x86_64 \
ar_mgr-1.0-0.42.g750eb1e.46101.x86_64 \
at-3.1.13-24.el7.x86_64 \
atk-2.28.1-1.el7.x86_64 \
attr-2.4.46-13.el7.x86_64 \
audit-2.8.4-4.el7.x86_64 \
audit-libs-2.8.4-4.el7.x86_64 \
augeas-libs-1.4.0-6.el7_6.1.x86_64 \
authconfig-6.2.8-30.el7.x86_64 \
autofs-5.0.7-99.el7.x86_64 \
autogen-libopts-5.18-5.el7.x86_64 \
avahi-glib-0.6.31-19.el7.x86_64 \
avahi-libs-0.6.31-19.el7.x86_64 \
basesystem-10.0-7.el7.centos.noarch \
bash-4.2.46-31.el7.x86_64 \
bash-completion-2.1-6.el7.noarch \
bc-1.06.95-13.el7.x86_64 \
bind-libs-9.9.4-74.el7_6.2.x86_64 \
bind-libs-lite-9.9.4-74.el7_6.2.x86_64 \
bind-license-9.9.4-74.el7_6.2.noarch \
bind-utils-9.9.4-74.el7_6.2.x86_64 \
binutils-2.27-34.base.el7.x86_64 \
biosdevname-0.7.3-1.el7.x86_64 \
blas-3.4.2-8.el7.x86_64 \
blktrace-1.0.5-8.el7.x86_64 \
boost-1.53.0-27.el7.x86_64 \
boost-atomic-1.53.0-27.el7.x86_64 \
boost-chrono-1.53.0-27.el7.x86_64 \
boost-context-1.53.0-27.el7.x86_64 \
boost-date-time-1.53.0-27.el7.x86_64 \
boost-filesystem-1.53.0-27.el7.x86_64 \
boost-graph-1.53.0-27.el7.x86_64 \
boost-iostreams-1.53.0-27.el7.x86_64 \
boost-locale-1.53.0-27.el7.x86_64 \
boost-math-1.53.0-27.el7.x86_64 \
boost-program-options-1.53.0-27.el7.x86_64 \
boost-python-1.53.0-27.el7.x86_64 \
boost-random-1.53.0-27.el7.x86_64 \
boost-regex-1.53.0-27.el7.x86_64 \
boost-serialization-1.53.0-27.el7.x86_64 \
boost-signals-1.53.0-27.el7.x86_64 \
boost-system-1.53.0-27.el7.x86_64 \
boost-test-1.53.0-27.el7.x86_64 \
boost-thread-1.53.0-27.el7.x86_64 \
boost-timer-1.53.0-27.el7.x86_64 \
boost-wave-1.53.0-27.el7.x86_64 \
bridge-utils-1.5-9.el7.x86_64 \
btrfs-progs-4.9.1-1.el7.x86_64 \
bzip2-1.0.6-13.el7.x86_64 \
bzip2-libs-1.0.6-13.el7.x86_64 \
ca-certificates-2018.2.22-70.0.el7_5.noarch \
cairo-1.15.12-3.el7.x86_64 \
c-ares-1.10.0-3.el7.x86_64 \
cc_mgr-1.0-0.41.g750eb1e.46101.x86_64 \
centos-indexhtml-7-9.el7.centos.noarch \
centos-logos-70.0.6-3.el7.centos.noarch \
centos-release-7-6.1810.2.el7.centos.x86_64 \
cgdcbxd-1.0.2-7.el7.x86_64 \
chkconfig-1.7.4-1.el7.x86_64 \
chrony-3.2-2.el7.x86_64 \
cifs-utils-6.2-10.el7.x86_64 \
compat-db47-4.7.25-28.el7.x86_64 \
compat-db-headers-4.7.25-28.el7.noarch \
compat-exiv2-023-0.23-2.el7.x86_64 \
compat-glibc-2.12-4.el7.centos.x86_64 \
compat-glibc-headers-2.12-4.el7.centos.x86_64 \
compat-libcap1-1.10-7.el7.x86_64 \
compat-libf2c-34-3.4.6-32.el7.x86_64 \
compat-libgfortran-41-4.1.2-44.el7.x86_64 \
compat-libtiff3-3.9.4-11.el7.x86_64 \
compat-openldap-2.3.43-5.el7.x86_64 \
copy-jdk-configs-3.3-10.el7_5.noarch \
coreutils-8.22-23.el7.x86_64 \
cpio-2.11-27.el7.x86_64 \
cpp-4.8.5-36.el7_6.2.x86_64 \
cracklib-2.9.0-11.el7.x86_64 \
cracklib-dicts-2.9.0-11.el7.x86_64 \
crash-7.2.3-8.el7.x86_64 \
crash-gcore-command-1.3.1-0.el7.x86_64 \
crash-ptdump-command-1.0.3-2.el7.x86_64 \
crash-trace-command-2.0-14.el7.x86_64 \
crda-3.18_2018.05.31-4.el7.x86_64 \
cronie-1.4.11-20.el7_6.x86_64 \
cronie-anacron-1.4.11-20.el7_6.x86_64 \
crontabs-1.11-6.20121102git.el7.noarch \
cryptsetup-2.0.3-3.el7.x86_64 \
cryptsetup-libs-2.0.3-3.el7.x86_64 \
cups-1.6.3-35.el7.x86_64 \
cups-client-1.6.3-35.el7.x86_64 \
cups-filesystem-1.6.3-35.el7.noarch \
cups-filters-1.0.35-22.el7.x86_64 \
cups-filters-libs-1.0.35-22.el7.x86_64 \
cups-libs-1.6.3-35.el7.x86_64 \
curl-7.29.0-51.el7_6.3.x86_64 \
cyrus-sasl-2.1.26-23.el7.x86_64 \
cyrus-sasl-lib-2.1.26-23.el7.x86_64 \
cyrus-sasl-plain-2.1.26-23.el7.x86_64 \
dapl-2.1.10mlnx-OFED.3.4.2.1.0.46101.x86_64 \
dapl-devel-2.1.10mlnx-OFED.3.4.2.1.0.46101.x86_64 \
dapl-devel-static-2.1.10mlnx-OFED.3.4.2.1.0.46101.x86_64 \
dapl-utils-2.1.10mlnx-OFED.3.4.2.1.0.46101.x86_64 \
datacenter-gpu-manager-1.7.1-1.x86_64 \
dbus-1.10.24-13.el7_6.x86_64 \
dbus-glib-0.100-7.el7.x86_64 \
dbus-libs-1.10.24-13.el7_6.x86_64 \
dbus-python-1.1.1-9.el7.x86_64 \
dejavu-fonts-common-2.33-6.el7.noarch \
dejavu-sans-fonts-2.33-6.el7.noarch \
desktop-file-utils-0.23-1.el7.x86_64 \
device-mapper-1.02.149-10.el7_6.8.x86_64 \
device-mapper-event-1.02.149-10.el7_6.8.x86_64 \
device-mapper-event-libs-1.02.149-10.el7_6.8.x86_64 \
device-mapper-libs-1.02.149-10.el7_6.8.x86_64 \
device-mapper-multipath-0.4.9-123.el7.x86_64 \
device-mapper-multipath-libs-0.4.9-123.el7.x86_64 \
device-mapper-persistent-data-0.7.3-3.el7.x86_64 \
dhclient-4.2.5-68.el7.centos.1.x86_64 \
dhcp-common-4.2.5-68.el7.centos.1.x86_64 \
dhcp-libs-4.2.5-68.el7.centos.1.x86_64 \
diffutils-3.3-4.el7.x86_64 \
DiskFree_Check-11.2.0-0.el5.x86_64 \
dkms-2.6.1-1.el7.noarch \
dmidecode-3.1-2.el7.x86_64 \
dmraid-1.0.0.rc16-28.el7.x86_64 \
dmraid-events-1.0.0.rc16-28.el7.x86_64 \
dos2unix-6.0.3-7.el7.x86_64 \
dosfstools-3.0.20-10.el7.x86_64 \
dracut-033-554.el7.x86_64 \
dracut-config-rescue-033-554.el7.x86_64 \
dracut-network-033-554.el7.x86_64 \
dump_pr-1.0-0.37.g750eb1e.46101.x86_64 \
dyninst-9.3.1-2.el7.x86_64 \
e2fsprogs-1.42.9-13.el7.x86_64 \
e2fsprogs-libs-1.42.9-13.el7.x86_64 \
ebtables-2.0.10-16.el7.x86_64 \
ed-1.9-4.el7.x86_64 \
efibootmgr-17-2.el7.x86_64 \
efivar-libs-36-11.el7_6.1.x86_64 \
elfutils-0.172-2.el7.x86_64 \
elfutils-default-yama-scope-0.172-2.el7.noarch \
elfutils-libelf-0.172-2.el7.x86_64 \
elfutils-libelf-devel-0.172-2.el7.x86_64 \
elfutils-libs-0.172-2.el7.x86_64 \
emacs-filesystem-24.3-22.el7.noarch \
ethtool-4.8-9.el7.x86_64 \
expat-2.1.0-10.el7_3.x86_64 \
file-5.11-35.el7.x86_64 \
file-libs-5.11-35.el7.x86_64 \
filesystem-3.2-25.el7.x86_64 \
findutils-4.5.11-6.el7.x86_64 \
fipscheck-1.4.1-6.el7.x86_64 \
fipscheck-lib-1.4.1-6.el7.x86_64 \
firewalld-0.5.3-5.el7.noarch \
firewalld-filesystem-0.5.3-5.el7.noarch \
flac-libs-1.3.0-5.el7_1.x86_64 \
fontconfig-2.13.0-4.3.el7.x86_64 \
fontpackages-filesystem-1.44-8.el7.noarch \
fprintd-0.8.1-2.el7.x86_64 \
fprintd-pam-0.8.1-2.el7.x86_64 \
freeglut-3.0.0-8.el7.x86_64 \
freetype-2.8-12.el7_6.1.x86_64 \
fribidi-1.0.2-1.el7.x86_64 \
ftp-0.17-67.el7.x86_64 \
fxload-2002_04_11-16.el7.x86_64 \
ganglia-3.7.2-2.el7.x86_64 \
ganglia-gmond-3.7.2-2.el7.x86_64 \
ganglia-gmond-python-3.7.2-2.el7.x86_64 \
gawk-4.0.2-4.el7_3.1.x86_64 \
gcc-4.8.5-36.el7_6.2.x86_64 \
gcc-gfortran-4.8.5-36.el7_6.2.x86_64 \
GConf2-3.2.6-8.el7.x86_64 \
gdb-7.6.1-114.el7.x86_64 \
gdbm-1.10-8.el7.x86_64 \
gdbm-devel-1.10-8.el7.x86_64 \
gdisk-0.8.10-2.el7.x86_64 \
gdk-pixbuf2-2.36.12-3.el7.x86_64 \
GeoIP-1.5.0-13.el7.x86_64 \
gettext-0.19.8.1-2.el7.x86_64 \
gettext-libs-0.19.8.1-2.el7.x86_64 \
ghostscript-9.07-31.el7_6.11.x86_64 \
ghostscript-cups-9.07-31.el7_6.11.x86_64 \
ghostscript-fonts-5.50-32.el7.noarch \
giflib-4.1.6-9.el7.x86_64 \
glib2-2.56.1-2.el7.x86_64 \
glibc-2.17-260.el7_6.6.i686 \
glibc-2.17-260.el7_6.6.x86_64 \
glibc-common-2.17-260.el7_6.6.x86_64 \
glibc-devel-2.17-260.el7_6.6.x86_64 \
glibc-headers-2.17-260.el7_6.6.x86_64 \
glx-utils-8.3.0-10.el7.x86_64 \
gmp-6.0.0-15.el7.x86_64 \
gnupg2-2.0.22-5.el7_5.x86_64 \
gnutls-3.3.29-9.el7_6.x86_64 \
gobject-introspection-1.56.1-1.el7.x86_64 \
gpgme-1.3.2-5.el7.x86_64 \
gpm-libs-1.20.7-5.el7.x86_64 \
graphite2-1.3.10-1.el7_3.x86_64 \
grep-2.20-3.el7.x86_64 \
groff-base-1.22.2-8.el7.x86_64 \
grub2-2.02-0.76.el7.centos.1.x86_64 \
grub2-common-2.02-0.76.el7.centos.1.noarch \
grub2-efi-x64-2.02-0.76.el7.centos.1.x86_64 \
grub2-pc-2.02-0.76.el7.centos.1.x86_64 \
grub2-pc-modules-2.02-0.76.el7.centos.1.noarch \
grub2-tools-2.02-0.76.el7.centos.1.x86_64 \
grub2-tools-extra-2.02-0.76.el7.centos.1.x86_64 \
grub2-tools-minimal-2.02-0.76.el7.centos.1.x86_64 \
grubby-8.28-25.el7.x86_64 \
gsm-1.0.13-11.el7.x86_64 \
gssproxy-0.7.0-21.el7.x86_64 \
gtk2-2.24.31-1.el7.x86_64 \
gtk-update-icon-cache-3.22.30-3.el7.x86_64 \
gzip-1.5-10.el7.x86_64 \
hardlink-1.0-19.el7.x86_64 \
harfbuzz-1.7.5-2.el7.x86_64 \
hcoll-4.3.2708-1.46101.x86_64 \
hesiod-3.2.1-3.el7.x86_64 \
hicolor-icon-theme-0.12-7.el7.noarch \
hostname-3.13-3.el7.x86_64 \
hpdiags-10.40.1953-1978.x86_64 \
hp-health-10.90-1873.8.rhel7.x86_64 \
hponcfg-5.4.0-0.x86_64 \
hp-scripting-tools-10.10-53.rhel6.x86_64 \
hwdata-0.252-9.1.el7.x86_64 \
ibacm-41mlnx1-OFED.4.3.3.0.0.46101.x86_64 \
ibdump-5.0.0-3.46101.x86_64 \
ibsim-0.7mlnx1-0.11.g85c342b.46101.x86_64 \
ibutils-1.5.7.1-0.12.gdcaeae2.46101.x86_64 \
ibutils2-2.1.1-0.104.MLNX20190408.gb55795e.46101.x86_64 \
Infiniband_Check-1.0.2-14.noarch \
infiniband-diags-5.4.0.MLNX20190422.d1468cd-0.1.46101.x86_64 \
infiniband-diags-compat-5.4.0.MLNX20190422.d1468cd-0.1.46101.x86_64 \
info-5.1-5.el7.x86_64 \
initscripts-9.49.46-1.el7.x86_64 \
ipmitool-1.8.18-7.el7.x86_64 \
iproute-4.11.0-14.el7_6.2.x86_64 \
iprutils-2.4.16.1-1.el7.x86_64 \
ipset-6.38-3.el7_6.x86_64 \
ipset-libs-6.38-3.el7_6.x86_64 \
iptables-1.4.21-28.el7.x86_64 \
iputils-20160308-10.el7.x86_64 \
irqbalance-1.0.7-11.el7.x86_64 \
iscsi-initiator-utils-6.2.0.874-10.el7.x86_64 \
iscsi-initiator-utils-iscsiuio-6.2.0.874-10.el7.x86_64 \
ivtv-firmware-20080701-26.el7.noarch \
iw-4.3-2.el7.x86_64 \
iwl1000-firmware-39.31.5.1-69.el7.noarch \
iwl100-firmware-39.31.5.1-69.el7.noarch \
iwl105-firmware-18.168.6.1-69.el7.noarch \
iwl135-firmware-18.168.6.1-69.el7.noarch \
iwl2000-firmware-18.168.6.1-69.el7.noarch \
iwl2030-firmware-18.168.6.1-69.el7.noarch \
iwl3160-firmware-22.0.7.0-69.el7.noarch \
iwl3945-firmware-15.32.2.9-69.el7.noarch \
iwl4965-firmware-228.61.2.24-69.el7.noarch \
iwl5000-firmware-8.83.5.1_1-69.el7.noarch \
iwl5150-firmware-8.24.2.2-69.el7.noarch \
iwl6000-firmware-9.221.4.1-69.el7.noarch \
iwl6000g2a-firmware-17.168.5.3-69.el7.noarch \
iwl6000g2b-firmware-17.168.5.2-69.el7.noarch \
iwl6050-firmware-41.28.5.1-69.el7.noarch \
iwl7260-firmware-22.0.7.0-69.el7.noarch \
iwl7265-firmware-22.0.7.0-69.el7.noarch \
jansson-2.10-1.el7.x86_64 \
jasper-libs-1.900.1-33.el7.x86_64 \
java-1.7.0-openjdk-1.7.0.231-2.6.19.1.el7_6.x86_64 \
java-1.7.0-openjdk-headless-1.7.0.231-2.6.19.1.el7_6.x86_64 \
java-1.8.0-openjdk-1.8.0.222.b10-0.el7_6.x86_64 \
java-1.8.0-openjdk-headless-1.8.0.222.b10-0.el7_6.x86_64 \
javapackages-tools-3.4.1-11.el7.noarch \
jbigkit-libs-2.0-11.el7.x86_64 \
json-c-0.11-4.el7_0.x86_64 \
kbd-1.15.5-15.el7.x86_64 \
kbd-legacy-1.15.5-15.el7.noarch \
kbd-misc-1.15.5-15.el7.noarch \
kernel-3.10.0-957.27.2.el7.x86_64 \
kernel-3.10.0-957.el7.x86_64 \
kernel-devel-3.10.0-957.27.2.el7.x86_64 \
kernel-devel-3.10.0-957.el7.x86_64 \
kernel-headers-3.10.0-957.27.2.el7.x86_64 \
kernel-tools-3.10.0-957.27.2.el7.x86_64 \
kernel-tools-libs-3.10.0-957.27.2.el7.x86_64 \
kexec-tools-2.0.15-21.el7_6.4.x86_64 \
keyutils-1.5.8-3.el7.x86_64 \
keyutils-libs-1.5.8-3.el7.x86_64 \
kmod-20-23.el7.x86_64 \
kmod-iser-4.6-OFED.4.6.1.0.1.1.ga2cfe08.rhel7u6.x86_64 \
kmod-isert-4.6-OFED.4.6.1.0.1.1.ga2cfe08.rhel7u6.x86_64 \
kmod-kernel-mft-mlnx-4.12.0-1.rhel7u6.x86_64 \
kmod-knem-1.1.3.90mlnx1-OFED.4.4.2.5.2.1.g993678c.rhel7u6.x86_64 \
kmod-libs-20-23.el7.x86_64 \
kmod-mlnx-ofa_kernel-4.6-OFED.4.6.1.0.1.1.ga2cfe08.rhel7u6.x86_64 \
kmod-nvidia-latest-dkms-418.87.00-2.el7.x86_64 \
kmod-rshim-1.6-0.g6aa30c7.rhel7u6.x86_64 \
kmod-srp-4.6-OFED.4.6.1.0.1.1.ga2cfe08.rhel7u6.x86_64 \
knem-1.1.3.90mlnx1-OFED.4.4.2.5.2.1.g993678c.rhel7u6.x86_64 \
kpartx-0.4.9-123.el7.x86_64 \
kpatch-0.6.1-1.el7.noarch \
krb5-libs-1.15.1-37.el7_6.x86_64 \
ksh-20120801-139.el7.x86_64 \
lapack-3.4.2-8.el7.x86_64 \
latrace-0.5.11-6.1.el7.x86_64 \
lcms2-2.6-3.el7.x86_64 \
ledmon-0.90-1.el7.x86_64 \
less-458-9.el7.x86_64 \
libacl-2.2.51-14.el7.x86_64 \
libaio-0.3.109-13.el7.x86_64 \
libarchive-3.1.2-10.el7_2.x86_64 \
libassuan-2.1.0-3.el7.x86_64 \
libasyncns-0.8-7.el7.x86_64 \
libattr-2.4.46-13.el7.x86_64 \
libbasicobjects-0.1.1-32.el7.x86_64 \
libblkid-2.23.2-59.el7_6.1.x86_64 \
libcap-2.22-9.el7.x86_64 \
libcap-ng-0.7.5-4.el7.x86_64 \
libcgroup-0.41-20.el7.x86_64 \
libcgroup-tools-0.41-20.el7.x86_64 \
libcollection-0.7.0-32.el7.x86_64 \
libcom_err-1.42.9-13.el7.x86_64 \
libconfig-1.4.9-5.el7.x86_64 \
libconfuse-2.7-7.el7.x86_64 \
libcroco-0.6.12-4.el7.x86_64 \
libcurl-7.29.0-51.el7_6.3.x86_64 \
libdaemon-0.14-7.el7.x86_64 \
libdb-5.3.21-24.el7.x86_64 \
libdb-devel-5.3.21-24.el7.x86_64 \
libdb-utils-5.3.21-24.el7.x86_64 \
libdmx-1.1.3-3.el7.x86_64 \
libdrm-2.4.91-3.el7.x86_64 \
libdwarf-20130207-4.el7.x86_64 \
libedit-3.0-12.20121213cvs.el7.x86_64 \
libepoxy-1.5.2-1.el7.x86_64 \
liberation-fonts-common-1.07.2-16.el7.noarch \
liberation-mono-fonts-1.07.2-16.el7.noarch \
libestr-0.1.9-2.el7.x86_64 \
libevdev-1.5.6-1.el7.x86_64 \
libevent-2.0.21-4.el7.x86_64 \
libfastjson-0.99.4-3.el7.x86_64 \
libffi-3.0.13-18.el7.x86_64 \
libfontenc-1.1.3-3.el7.x86_64 \
libfprint-0.8.2-1.el7.x86_64 \
libgcc-4.8.5-36.el7_6.2.i686 \
libgcc-4.8.5-36.el7_6.2.x86_64 \
libgcrypt-1.5.3-14.el7.x86_64 \
libgfortran-4.8.5-36.el7_6.2.x86_64 \
libglvnd-1.0.1-0.8.git5baa1e5.el7.x86_64 \
libglvnd-egl-1.0.1-0.8.git5baa1e5.el7.x86_64 \
libglvnd-gles-1.0.1-0.8.git5baa1e5.el7.x86_64 \
libglvnd-glx-1.0.1-0.8.git5baa1e5.el7.x86_64 \
libglvnd-opengl-1.0.1-0.8.git5baa1e5.el7.x86_64 \
libgomp-4.8.5-36.el7_6.2.x86_64 \
libgpg-error-1.12-3.el7.x86_64 \
libibcm-41mlnx1-OFED.4.1.0.1.0.46101.x86_64 \
libibcm-devel-41mlnx1-OFED.4.1.0.1.0.46101.x86_64 \
libibmad-5.4.0.MLNX20190423.1d917ae-0.1.46101.x86_64 \
libibmad-devel-5.4.0.MLNX20190423.1d917ae-0.1.46101.x86_64 \
libibmad-static-5.4.0.MLNX20190423.1d917ae-0.1.46101.x86_64 \
libibumad-43.1.1.MLNX20190422.87b4d9b-0.1.46101.x86_64 \
libibumad-devel-43.1.1.MLNX20190422.87b4d9b-0.1.46101.x86_64 \
libibumad-static-43.1.1.MLNX20190422.87b4d9b-0.1.46101.x86_64 \
libibverbs-41mlnx1-OFED.4.6.0.4.1.46101.x86_64 \
libibverbs-devel-41mlnx1-OFED.4.6.0.4.1.46101.x86_64 \
libibverbs-devel-static-41mlnx1-OFED.4.6.0.4.1.46101.x86_64 \
libibverbs-utils-41mlnx1-OFED.4.6.0.4.1.46101.x86_64 \
libICE-1.0.9-9.el7.x86_64 \
libicu-50.1.2-17.el7.x86_64 \
libidn-1.28-4.el7.x86_64 \
libini_config-1.3.1-32.el7.x86_64 \
libjpeg-turbo-1.2.90-6.el7.x86_64 \
libldb-1.3.4-1.el7.x86_64 \
libmcrypt-2.5.8-13.el7.x86_64 \
libmlx4-41mlnx1-OFED.4.5.0.0.3.46101.x86_64 \
libmlx4-devel-41mlnx1-OFED.4.5.0.0.3.46101.x86_64 \
libmlx5-41mlnx1-OFED.4.6.0.0.4.46101.x86_64 \
libmlx5-devel-41mlnx1-OFED.4.6.0.0.4.46101.x86_64 \
libmnl-1.0.3-7.el7.x86_64 \
libmodman-2.0.1-8.el7.x86_64 \
libmount-2.23.2-59.el7_6.1.x86_64 \
libmpc-1.0.1-3.el7.x86_64 \
libnetfilter_conntrack-1.0.6-1.el7_3.x86_64 \
libnfnetlink-1.0.1-4.el7.x86_64 \
libnfsidmap-0.25-19.el7.x86_64 \
libnl-1.1.4-3.el7.x86_64 \
libnl3-3.2.28-4.el7.x86_64 \
libnl3-cli-3.2.28-4.el7.x86_64 \
libogg-1.3.0-7.el7.x86_64 \
libpath_utils-0.2.1-32.el7.x86_64 \
libpcap-1.5.3-11.el7.x86_64 \
libpciaccess-0.14-1.el7.x86_64 \
libpfm-4.7.0-10.el7.x86_64 \
libpipeline-1.2.3-3.el7.x86_64 \
libpng12-1.2.50-10.el7.x86_64 \
libpng-1.5.13-7.el7_2.x86_64 \
libproxy-0.4.11-11.el7.x86_64 \
libpwquality-1.2.3-5.el7.x86_64 \
libquadmath-4.8.5-36.el7_6.2.x86_64 \
libquadmath-devel-4.8.5-36.el7_6.2.x86_64 \
librados2-10.2.5-4.el7.x86_64 \
librdmacm-41mlnx1-OFED.4.6.0.0.1.46101.x86_64 \
librdmacm-devel-41mlnx1-OFED.4.6.0.0.1.46101.x86_64 \
librdmacm-utils-41mlnx1-OFED.4.6.0.0.1.46101.x86_64 \
libref_array-0.1.5-32.el7.x86_64 \
libreport-2.1.11-42.el7.centos.x86_64 \
libreport-centos-2.1.11-42.el7.centos.x86_64 \
libreport-cli-2.1.11-42.el7.centos.x86_64 \
libreport-filesystem-2.1.11-42.el7.centos.x86_64 \
libreport-plugin-mailx-2.1.11-42.el7.centos.x86_64 \
libreport-plugin-mantisbt-2.1.11-42.el7.centos.x86_64 \
libreport-plugin-rhtsupport-2.1.11-42.el7.centos.x86_64 \
libreport-plugin-ureport-2.1.11-42.el7.centos.x86_64 \
libreport-python-2.1.11-42.el7.centos.x86_64 \
libreport-web-2.1.11-42.el7.centos.x86_64 \
librxe-41mlnx1-OFED.4.4.2.4.6.46101.x86_64 \
librxe-devel-static-41mlnx1-OFED.4.4.2.4.6.46101.x86_64 \
libseccomp-2.3.1-3.el7.x86_64 \
libselinux-2.5-14.1.el7.x86_64 \
libselinux-python-2.5-14.1.el7.x86_64 \
libselinux-utils-2.5-14.1.el7.x86_64 \
libsemanage-2.5-14.el7.x86_64 \
libsepol-2.5-10.el7.x86_64 \
libSM-1.2.2-2.el7.x86_64 \
libsmartcols-2.23.2-59.el7_6.1.x86_64 \
libsmbclient-4.8.3-6.el7_6.x86_64 \
libsmi-0.4.8-13.el7.x86_64 \
libsndfile-1.0.25-10.el7.x86_64 \
libss-1.42.9-13.el7.x86_64 \
libssh2-1.4.3-12.el7_6.3.x86_64 \
libsss_idmap-1.16.2-13.el7_6.8.x86_64 \
libsss_nss_idmap-1.16.2-13.el7_6.8.x86_64 \
libstdc++-4.8.5-36.el7_6.2.i686 \
libstdc++-4.8.5-36.el7_6.2.x86_64 \
libstoragemgmt-1.6.2-4.el7.x86_64 \
libstoragemgmt-python-1.6.2-4.el7.noarch \
libstoragemgmt-python-clibs-1.6.2-4.el7.x86_64 \
libsysfs-2.1.0-16.el7.x86_64 \
libtalloc-2.1.13-1.el7.x86_64 \
libtar-1.2.11-29.el7.x86_64 \
libtasn1-4.10-1.el7.x86_64 \
libtdb-1.3.15-1.el7.x86_64 \
libteam-1.27-6.el7_6.1.x86_64 \
libtevent-0.9.36-1.el7.x86_64 \
libthai-0.1.14-9.el7.x86_64 \
libtiff-4.0.3-27.el7_3.x86_64 \
libtirpc-0.2.4-0.15.el7.x86_64 \
libtool-ltdl-2.4.2-22.el7_3.x86_64 \
libunistring-0.9.3-9.el7.x86_64 \
libusbx-1.0.21-1.el7.x86_64 \
libuser-0.60-9.el7.x86_64 \
libutempter-1.1.6-4.el7.x86_64 \
libuuid-2.23.2-59.el7_6.1.x86_64 \
libvdpau-1.1.1-3.el7.x86_64 \
libverto-0.2.5-4.el7.x86_64 \
libverto-libevent-0.2.5-4.el7.x86_64 \
libvorbis-1.3.3-8.el7.1.x86_64 \
libwayland-client-1.15.0-1.el7.x86_64 \
libwayland-server-1.15.0-1.el7.x86_64 \
libwbclient-4.8.3-6.el7_6.x86_64 \
libX11-1.6.5-2.el7.x86_64 \
libX11-common-1.6.5-2.el7.noarch \
libx86-1.1-15.el7.x86_64 \
libXau-1.0.8-2.1.el7.x86_64 \
libXaw-1.0.13-4.el7.x86_64 \
libxcb-1.13-1.el7.x86_64 \
libXcomposite-0.4.4-4.1.el7.x86_64 \
libXcursor-1.1.15-1.el7.x86_64 \
libXdamage-1.1.4-4.1.el7.x86_64 \
libXdmcp-1.1.2-6.el7.x86_64 \
libXext-1.3.3-3.el7.x86_64 \
libXfixes-5.0.3-1.el7.x86_64 \
libXfont-1.5.4-1.el7.x86_64 \
libXfont2-2.0.3-1.el7.x86_64 \
libXft-2.3.2-2.el7.x86_64 \
libXi-1.7.9-1.el7.x86_64 \
libXinerama-1.1.3-2.1.el7.x86_64 \
libxkbfile-1.0.9-3.el7.x86_64 \
libxml2-2.9.1-6.el7_2.3.i686 \
libxml2-2.9.1-6.el7_2.3.x86_64 \
libxml2-python-2.9.1-6.el7_2.3.x86_64 \
libXmu-1.1.2-2.el7.x86_64 \
libXp-1.0.2-2.1.el7.x86_64 \
libXpm-3.5.12-1.el7.x86_64 \
libXrandr-1.5.1-2.el7.x86_64 \
libXrender-0.9.10-1.el7.x86_64 \
libxshmfence-1.2-1.el7.x86_64 \
libxslt-1.1.28-5.el7.x86_64 \
libXt-1.1.5-3.el7.x86_64 \
libXtst-1.2.3-1.el7.x86_64 \
libXv-1.0.11-1.el7.x86_64 \
libXvMC-1.0.10-1.el7.x86_64 \
libXxf86dga-1.1.4-2.1.el7.x86_64 \
libXxf86misc-1.0.3-7.1.el7.x86_64 \
libXxf86vm-1.1.4-1.el7.x86_64 \
libyaml-0.1.4-11.el7_0.x86_64 \
linux-firmware-20180911-69.git85c5d90.el7.noarch \
lksctp-tools-1.0.17-2.el7.x86_64 \
lldpad-1.0.1-3.git036e314.el7.x86_64 \
llvm-private-6.0.1-2.el7.x86_64 \
lm_sensors-libs-3.4.0-6.20160601gitf9185e5.el7.x86_64 \
logrotate-3.8.6-17.el7.x86_64 \
logwatch-7.4.0-35.20130522svn140.el7_5.noarch \
lshw-B.02.18-12.el7.x86_64 \
lsof-4.87-6.el7.x86_64 \
lsscsi-0.27-6.el7.x86_64 \
ltrace-0.7.91-15.el7.x86_64 \
lua-5.1.4-15.el7.x86_64 \
lvm2-2.02.180-10.el7_6.8.x86_64 \
lvm2-libs-2.02.180-10.el7_6.8.x86_64 \
lz4-1.7.5-2.el7.x86_64 \
lzo-2.06-8.el7.x86_64 \
m4-1.4.16-10.el7.x86_64 \
mailcap-2.1.41-2.el7.noarch \
mailx-12.5-19.el7.x86_64 \
make-3.82-23.el7.x86_64 \
man-db-2.6.3-11.el7.x86_64 \
man-pages-3.53-5.el7.noarch \
man-pages-overrides-7.6.2-1.el7.x86_64 \
mariadb-libs-5.5.60-1.el7_5.x86_64 \
mcelog-144-9.94d853b2ea81.el7.x86_64 \
mdadm-4.1-rc1_2.el7.x86_64 \
memstomp-0.1.4-11.el7.x86_64 \
mesa-dri-drivers-18.0.5-4.el7_6.x86_64 \
mesa-filesystem-18.0.5-4.el7_6.x86_64 \
mesa-libEGL-18.0.5-4.el7_6.x86_64 \
mesa-libgbm-18.0.5-4.el7_6.x86_64 \
mesa-libGL-18.0.5-4.el7_6.x86_64 \
mesa-libglapi-18.0.5-4.el7_6.x86_64 \
mesa-libxatracker-18.0.5-4.el7_6.x86_64 \
mesa-private-llvm-3.9.1-3.el7.x86_64 \
mft-4.12.0-105.x86_64 \
microcode_ctl-2.1-47.4.el7_6.x86_64 \
mlnx-ethtool-4.19-1.46101.x86_64 \
mlnx-fw-updater-4.6-1.0.1.1.x86_64 \
mlnx-ofa_kernel-4.6-OFED.4.6.1.0.1.1.ga2cfe08.rhel7u6.x86_64 \
mlnx-ofa_kernel-devel-4.6-OFED.4.6.1.0.1.1.ga2cfe08.rhel7u6.x86_64 \
mlnxofed-docs-4.6-1.0.1.1.noarch \
mlocate-0.26-8.el7.x86_64 \
mokutil-15-2.el7.centos.x86_64 \
monitor-edid-3.0-6.el7.x86_64 \
motif-2.3.4-14.el7_5.x86_64 \
mozjs17-17.0.0-20.el7.x86_64 \
mpfr-3.1.1-4.el7.x86_64 \
mpi-selector-1.0.3-1.46101.x86_64 \
mpitests_openmpi-3.2.20-e1a0676.46101.x86_64 \
mstflint-4.11.0-1.14.g840c9c2.46101.x86_64 \
mtdev-1.1.5-5.el7.x86_64 \
mtools-4.0.18-5.el7.x86_64 \
mtr-0.85-7.el7.x86_64 \
mt-st-1.1-14.el7.x86_64 \
mxm-3.7.3111-1.46101.x86_64 \
nagios-nsca-client-2.7.2-3.el6.rf.x86_64 \
nano-2.3.1-10.el7.x86_64 \
ncurses-5.9-14.20130511.el7_4.x86_64 \
ncurses-base-5.9-14.20130511.el7_4.noarch \
ncurses-libs-5.9-14.20130511.el7_4.i686 \
ncurses-libs-5.9-14.20130511.el7_4.x86_64 \
net-snmp-5.7.2-38.el7_6.2.x86_64 \
net-snmp-agent-libs-5.7.2-38.el7_6.2.x86_64 \
net-snmp-libs-5.7.2-38.el7_6.2.x86_64 \
net-snmp-utils-5.7.2-38.el7_6.2.x86_64 \
nettle-2.7.1-8.el7.x86_64 \
net-tools-2.0-0.24.20131004git.el7.x86_64 \
newt-0.52.15-4.el7.x86_64 \
newt-python-0.52.15-4.el7.x86_64 \
nfs4-acl-tools-0.3.3-19.el7.x86_64 \
nfs-utils-1.3.0-0.61.el7.x86_64 \
nmap-6.40-16.el7.x86_64 \
nmap-ncat-6.40-16.el7.x86_64 \
nspr-4.19.0-1.el7_5.x86_64 \
nss-3.36.0-7.1.el7_6.x86_64 \
nss-pem-1.0.3-5.el7_6.1.x86_64 \
nss-softokn-3.36.0-5.el7_5.x86_64 \
nss-softokn-freebl-3.36.0-5.el7_5.i686 \
nss-softokn-freebl-3.36.0-5.el7_5.x86_64 \
nss-sysinit-3.36.0-7.1.el7_6.x86_64 \
nss-tools-3.36.0-7.1.el7_6.x86_64 \
nss-util-3.36.0-1.1.el7_6.x86_64 \
ntp-4.2.6p5-28.el7.centos.x86_64 \
ntpdate-4.2.6p5-28.el7.centos.x86_64 \
ntsysv-1.7.4-1.el7.x86_64 \
numactl-2.0.9-7.el7.x86_64 \
numactl-libs-2.0.9-7.el7.x86_64 \
nvidia-driver-latest-dkms-418.87.00-2.el7.x86_64 \
nvidia-driver-latest-dkms-cuda-418.87.00-2.el7.x86_64 \
nvidia-driver-latest-dkms-cuda-libs-418.87.00-2.el7.x86_64 \
nvidia-driver-latest-dkms-devel-418.87.00-2.el7.x86_64 \
nvidia-driver-latest-dkms-libs-418.87.00-2.el7.x86_64 \
nvidia-driver-latest-dkms-NvFBCOpenGL-418.87.00-2.el7.x86_64 \
nvidia-driver-latest-dkms-NVML-418.87.00-2.el7.x86_64 \
nvidia-modprobe-latest-dkms-418.87.00-2.el7.x86_64 \
nvidia-persistenced-latest-dkms-418.87.00-2.el7.x86_64 \
nvidia-xconfig-latest-dkms-418.87.00-2.el7.x86_64 \
ocmcontroller-201901-1.noarch \
ocsinventory-agent-2.0.5-8.wg.el7.x86_64 \
ocsinventory-wgext-1.6-6.el7.noarch \
ofed-scripts-4.6-OFED.4.6.1.0.1.x86_64 \
OpenIPMI-2.0.23-2.el7.x86_64 \
OpenIPMI-libs-2.0.23-2.el7.x86_64 \
OpenIPMI-modalias-2.0.23-2.el7.x86_64 \
openjpeg-libs-1.5.1-18.el7.x86_64 \
openldap-2.4.44-21.el7_6.x86_64 \
openmpi-4.0.2a1-1.46101.x86_64 \
opensm-5.4.0.MLNX20190422.ed81811-0.1.46101.x86_64 \
opensm-devel-5.4.0.MLNX20190422.ed81811-0.1.46101.x86_64 \
opensm-libs-5.4.0.MLNX20190422.ed81811-0.1.46101.x86_64 \
opensm-static-5.4.0.MLNX20190422.ed81811-0.1.46101.x86_64 \
openssh-7.4p1-16.el7.x86_64 \
openssh-clients-7.4p1-16.el7.x86_64 \
openssh-server-7.4p1-16.el7.x86_64 \
openssl098e-0.9.8e-29.el7.centos.3.x86_64 \
openssl-1.0.2k-16.el7_6.1.x86_64 \
openssl-libs-1.0.2k-16.el7_6.1.x86_64 \
os-prober-1.58-9.el7.x86_64 \
p11-kit-0.23.5-3.el7.x86_64 \
p11-kit-trust-0.23.5-3.el7.x86_64 \
pam-1.1.8-22.el7.x86_64 \
pango-1.42.4-2.el7_6.x86_64 \
papi-5.2.0-26.el7.x86_64 \
parted-3.1-29.el7.x86_64 \
passwd-0.79-4.el7.x86_64 \
patch-2.7.1-10.el7_5.x86_64 \
pciutils-3.5.1-3.el7.x86_64 \
pciutils-libs-3.5.1-3.el7.x86_64 \
pcre-8.32-17.el7.x86_64 \
pcsc-lite-libs-1.8.8-8.el7.x86_64 \
perftest-4.4-0.5.g1ceab48.46101.x86_64 \
perl-5.16.3-294.el7_6.x86_64 \
perl-Archive-Extract-0.68-3.el7.noarch \
perl-Archive-Tar-1.92-2.el7.noarch \
perl-autodie-2.16-2.el7.noarch \
perl-Bit-Vector-7.3-3.el7.x86_64 \
perl-B-Lint-1.17-3.el7.noarch \
perl-Business-ISBN-2.06-2.el7.noarch \
perl-Business-ISBN-Data-20120719.001-2.el7.noarch \
perl-Carp-1.26-244.el7.noarch \
perl-Carp-Clan-6.04-10.el7.noarch \
perl-CGI-3.63-4.el7.noarch \
perl-Compress-Raw-Bzip2-2.061-3.el7.x86_64 \
perl-Compress-Raw-Zlib-2.061-4.el7.x86_64 \
perl-constant-1.27-2.el7.noarch \
perl-Convert-ASN1-0.26-4.el7.noarch \
perl-core-5.16.3-294.el7_6.x86_64 \
perl-CPAN-1.9800-294.el7_6.noarch \
perl-CPAN-Meta-2.120921-5.el7.noarch \
perl-CPAN-Meta-Requirements-2.122-7.el7.noarch \
perl-CPAN-Meta-YAML-0.008-14.el7.noarch \
perl-CPANPLUS-0.91.38-4.el7.noarch \
perl-CPANPLUS-Dist-Build-0.70-3.el7.noarch \
perl-Crypt-SSLeay-0.64-5.el7.x86_64 \
perl-Data-Dumper-2.145-3.el7.x86_64 \
perl-Date-Calc-6.3-14.el7.noarch \
perl-Date-Manip-6.41-2.el7.noarch \
perl-DBD-SQLite-1.39-3.el7.x86_64 \
perl-DB_File-1.830-6.el7.x86_64 \
perl-DBI-1.627-4.el7.x86_64 \
perl-DBIx-Simple-1.35-7.el7.noarch \
perl-devel-5.16.3-294.el7_6.x86_64 \
perl-Digest-1.17-245.el7.noarch \
perl-Digest-MD5-2.52-3.el7.x86_64 \
perl-Digest-SHA-5.85-4.el7.x86_64 \
perl-Encode-2.51-7.el7.x86_64 \
perl-Encode-Locale-1.03-5.el7.noarch \
perl-Env-1.04-2.el7.noarch \
perl-Exporter-5.68-3.el7.noarch \
perl-ExtUtils-CBuilder-0.28.2.6-294.el7_6.noarch \
perl-ExtUtils-Embed-1.30-294.el7_6.noarch \
perl-ExtUtils-Install-1.58-294.el7_6.noarch \
perl-ExtUtils-MakeMaker-6.68-3.el7.noarch \
perl-ExtUtils-Manifest-1.61-244.el7.noarch \
perl-ExtUtils-ParseXS-3.18-3.el7.noarch \
perl-FCGI-0.74-8.el7.x86_64 \
perl-File-CheckTree-4.42-3.el7.noarch \
perl-File-Fetch-0.42-2.el7.noarch \
perl-File-Listing-6.04-7.el7.noarch \
perl-File-Path-2.09-2.el7.noarch \
perl-File-Temp-0.23.01-3.el7.noarch \
perl-Filter-1.49-3.el7.x86_64 \
perl-Font-AFM-1.20-13.el7.noarch \
perl-Getopt-Long-2.40-3.el7.noarch \
perl-HTML-Format-2.10-7.el7.noarch \
perl-HTML-Parser-3.71-4.el7.x86_64 \
perl-HTML-Tagset-3.20-15.el7.noarch \
perl-HTML-Tree-5.03-2.el7.noarch \
perl-HTTP-Cookies-6.01-5.el7.noarch \
perl-HTTP-Daemon-6.01-8.el7.noarch \
perl-HTTP-Date-6.02-8.el7.noarch \
perl-HTTP-Message-6.06-6.el7.noarch \
perl-HTTP-Negotiate-6.01-5.el7.noarch \
perl-HTTP-Tiny-0.033-3.el7.noarch \
perl-IO-Compress-2.061-2.el7.noarch \
perl-IO-HTML-1.00-2.el7.noarch \
perl-IO-Socket-IP-0.21-5.el7.noarch \
perl-IO-Socket-SSL-1.94-7.el7.noarch \
perl-IO-stringy-2.110-22.el7.noarch \
perl-IO-Zlib-1.10-294.el7_6.noarch \
perl-IPC-Cmd-0.80-4.el7.noarch \
perl-JSON-PP-2.27202-2.el7.noarch \
perl-libs-5.16.3-294.el7_6.x86_64 \
perl-libwww-perl-6.05-2.el7.noarch \
perl-libxml-perl-0.08-19.el7.noarch \
perl-Locale-Codes-3.26-2.el7.noarch \
perl-Locale-Maketext-1.23-3.el7.noarch \
perl-Locale-Maketext-Simple-0.21-294.el7_6.noarch \
perl-local-lib-1.008010-4.el7.noarch \
perl-Log-Message-0.08-3.el7.noarch \
perl-Log-Message-Simple-0.10-2.el7.noarch \
perl-LWP-MediaTypes-6.02-2.el7.noarch \
perl-macros-5.16.3-294.el7_6.x86_64 \
perl-Module-Build-0.40.05-2.el7.noarch \
perl-Module-CoreList-2.76.02-294.el7_6.noarch \
perl-Module-Load-0.24-3.el7.noarch \
perl-Module-Load-Conditional-0.54-3.el7.noarch \
perl-Module-Loaded-0.08-294.el7_6.noarch \
perl-Module-Metadata-1.000018-2.el7.noarch \
perl-Module-Pluggable-4.8-3.el7.noarch \
perl-Mozilla-CA-20130114-5.el7.noarch \
perl-Net-Daemon-0.48-5.el7.noarch \
perl-Net-HTTP-6.06-2.el7.noarch \
perl-Net-IP-1.26-4.el7.noarch \
perl-Net-LibIDN-0.12-15.el7.x86_64 \
perl-Net-SSLeay-1.55-6.el7.x86_64 \
perl-Object-Accessor-0.42-294.el7_6.noarch \
perl-OLE-Storage_Lite-0.19-9.el7.noarch \
perl-Package-Constants-0.02-294.el7_6.noarch \
perl-Params-Check-0.38-2.el7.noarch \
perl-parent-0.225-244.el7.noarch \
perl-Parse-CPAN-Meta-1.4404-5.el7.noarch \
perl-Parse-RecDescent-1.967009-5.el7.noarch \
perl-PathTools-3.40-5.el7.x86_64 \
perl-Perl-OSType-1.003-3.el7.noarch \
perl-PlRPC-0.2020-14.el7.noarch \
perl-Pod-Checker-1.60-2.el7.noarch \
perl-Pod-Escapes-1.04-294.el7_6.noarch \
perl-Pod-LaTeX-0.61-2.el7.noarch \
perl-podlators-2.5.1-3.el7.noarch \
perl-Pod-Parser-1.61-2.el7.noarch \
perl-Pod-Perldoc-3.20-4.el7.noarch \
perl-Pod-Simple-3.28-4.el7.noarch \
perl-Pod-Usage-1.63-3.el7.noarch \
perl-Proc-Daemon-0.19-1.el7.noarch \
perl-Proc-ProcessTable-0.48-1.el7.x86_64 \
perl-Scalar-List-Utils-1.27-248.el7.x86_64 \
perl-Socket-2.010-4.el7.x86_64 \
perl-Spreadsheet-WriteExcel-2.40-1.el7.noarch \
perl-Storable-2.45-3.el7.x86_64 \
perl-Sys-CPU-0.54-4.el7.x86_64 \
perl-Sys-MemInfo-0.91-7.el7.x86_64 \
perl-Sys-Syslog-0.33-3.el7.x86_64 \
perl-Term-UI-0.36-2.el7.noarch \
perl-Test-Harness-3.28-3.el7.noarch \
perl-Test-Simple-0.98-243.el7.noarch \
perl-Text-ParseWords-3.29-4.el7.noarch \
perl-Text-Soundex-3.04-4.el7.x86_64 \
perl-Text-Unidecode-0.04-20.el7.noarch \
perl-Thread-Queue-3.02-2.el7.noarch \
perl-threads-1.87-4.el7.x86_64 \
perl-threads-shared-1.43-6.el7.x86_64 \
perl-TimeDate-2.30-2.el7.noarch \
perl-Time-HiRes-1.9725-3.el7.x86_64 \
perl-Time-Local-1.2300-2.el7.noarch \
perl-Time-Piece-1.20.1-294.el7_6.x86_64 \
perl-URI-1.60-9.el7.noarch \
perl-version-0.99.07-3.el7.x86_64 \
perl-Version-Requirements-0.101022-244.el7.noarch \
perl-WWW-RobotRules-6.02-5.el7.noarch \
perl-XML-Dumper-0.81-17.el7.noarch \
perl-XML-Grove-0.46alpha-52.el7.noarch \
perl-XML-LibXML-2.0018-5.el7.x86_64 \
perl-XML-NamespaceSupport-1.11-10.el7.noarch \
perl-XML-Parser-2.41-10.el7.x86_64 \
perl-XML-SAX-0.99-9.el7.noarch \
perl-XML-SAX-Base-1.08-7.el7.noarch \
perl-XML-Simple-2.20-5.el7.noarch \
perl-XML-Simple-DTDReader-0.04-16.el7.noarch \
perl-XML-Twig-3.44-2.el7.noarch \
pinentry-0.8.1-17.el7.x86_64 \
pinfo-0.6.10-9.el7.x86_64 \
pixman-0.34.0-1.el7.x86_64 \
pkgconfig-0.27.1-4.el7.x86_64 \
plymouth-0.8.9-0.31.20140113.el7.centos.x86_64 \
plymouth-core-libs-0.8.9-0.31.20140113.el7.centos.x86_64 \
plymouth-scripts-0.8.9-0.31.20140113.el7.centos.x86_64 \
pm-utils-1.4.1-27.el7.x86_64 \
policycoreutils-2.5-29.el7_6.1.x86_64 \
polkit-0.112-18.el7_6.1.x86_64 \
polkit-pkla-compat-0.1-4.el7.x86_64 \
poppler-0.26.5-20.el7.x86_64 \
poppler-data-0.4.6-3.el7.noarch \
poppler-utils-0.26.5-20.el7.x86_64 \
popt-1.13-16.el7.x86_64 \
portreserve-0.0.5-11.el7.x86_64 \
postfix-2.10.1-7.el7.x86_64 \
prelink-0.5.0-9.el7.x86_64 \
procmail-3.22-36.el7_4.1.x86_64 \
procps-ng-3.3.10-23.el7.x86_64 \
psacct-6.6.1-13.el7.x86_64 \
ps_mem-3.1-7.el7.noarch \
psmisc-22.20-15.el7.x86_64 \
pth-2.0.7-23.el7.x86_64 \
pulseaudio-libs-10.0-5.el7.x86_64 \
pygobject2-2.28.6-11.el7.x86_64 \
pygpgme-0.3-9.el7.x86_64 \
pyliblzma-0.5.3-11.el7.x86_64 \
pyparsing-1.5.6-9.el7.noarch \
python-2.7.5-80.el7_6.x86_64 \
python2-futures-3.1.1-5.el7.noarch \
python-augeas-0.5.0-2.el7.noarch \
python-chardet-2.2.1-1.el7_1.noarch \
python-configobj-4.7.2-7.el7.noarch \
python-configshell-1.1.fb23-5.el7.noarch \
python-decorator-3.4.0-3.el7.noarch \
python-dmidecode-3.12.2-3.el7.x86_64 \
python-ethtool-0.8-7.el7.x86_64 \
python-firewall-0.5.3-5.el7.noarch \
python-gobject-base-3.22.0-1.el7_4.1.x86_64 \
python-iniparse-0.4-9.el7.noarch \
python-javapackages-3.4.1-11.el7.noarch \
python-kitchen-1.1.1-5.el7.noarch \
python-kmod-0.9-4.el7.x86_64 \
python-libs-2.7.5-80.el7_6.x86_64 \
python-linux-procfs-0.4.9-4.el7.noarch \
python-lxml-3.2.1-4.el7.x86_64 \
python-perf-3.10.0-957.27.2.el7.x86_64 \
python-pycurl-7.19.0-19.el7.x86_64 \
python-pyudev-0.15-9.el7.noarch \
python-rtslib-2.1.fb63-13.el7.noarch \
python-schedutils-0.4-6.el7.x86_64 \
python-six-1.9.0-2.el7.noarch \
python-slip-0.4.0-4.el7.noarch \
python-slip-dbus-0.4.0-4.el7.noarch \
python-urlgrabber-3.10-9.el7.noarch \
python-urwid-1.1.1-3.el7.x86_64 \
pyxattr-0.5.1-5.el7.x86_64 \
qpdf-libs-5.0.1-3.el7.x86_64 \
qperf-0.4.9-9.46101.x86_64 \
qrencode-libs-3.4.1-3.el7.x86_64 \
quota-4.01-17.el7.x86_64 \
quota-nls-4.01-17.el7.noarch \
rasdaemon-0.4.1-33.1.el7_5.x86_64 \
readline-6.2-10.el7.x86_64 \
rfkill-0.4-10.el7.x86_64 \
rmt-1.5.2-13.el7.x86_64 \
rng-tools-6.3.1-3.el7.x86_64 \
rootfiles-8.1-11.el7.noarch \
rpcbind-0.2.0-47.el7.x86_64 \
rpm-4.11.3-35.el7.x86_64 \
rpm-build-libs-4.11.3-35.el7.x86_64 \
rpm-libs-4.11.3-35.el7.x86_64 \
rpm-python-4.11.3-35.el7.x86_64 \
rsync-3.1.2-4.el7.x86_64 \
rsyslog-8.24.0-34.el7.x86_64 \
ruby-2.0.0.648-35.el7_6.x86_64 \
rubygem-abrt-0.3.0-1.el7.noarch \
rubygem-bigdecimal-1.2.0-35.el7_6.x86_64 \
rubygem-io-console-0.4.2-35.el7_6.x86_64 \
rubygem-json-1.7.7-35.el7_6.x86_64 \
rubygem-psych-2.0.0-35.el7_6.x86_64 \
rubygem-rdoc-4.0.0-35.el7_6.noarch \
rubygems-2.0.14.1-35.el7_6.noarch \
ruby-irb-2.0.0.648-35.el7_6.noarch \
ruby-libs-2.0.0.648-35.el7_6.x86_64 \
samba-client-4.8.3-6.el7_6.x86_64 \
samba-client-libs-4.8.3-6.el7_6.x86_64 \
samba-common-4.8.3-6.el7_6.noarch \
samba-common-libs-4.8.3-6.el7_6.x86_64 \
satyr-0.13-15.el7.x86_64 \
scl-utils-20130529-19.el7.x86_64 \
screen-4.1.0-0.25.20120314git3c2946.el7.x86_64 \
sed-4.2.2-5.el7.x86_64 \
selinux-policy-3.13.1-229.el7_6.15.noarch \
selinux-policy-targeted-3.13.1-229.el7_6.15.noarch \
sendmail-8.14.7-5.el7.x86_64 \
sendmail-cf-8.14.7-5.el7.noarch \
setserial-2.17-33.el7.x86_64 \
setup-2.8.71-10.el7.noarch \
setuptool-1.19.11-8.el7.x86_64 \
sg3_utils-1.37-17.el7.x86_64 \
sg3_utils-libs-1.37-17.el7.x86_64 \
sgpio-1.2.0.10-13.el7.x86_64 \
shadow-utils-4.1.5.1-25.el7_6.1.x86_64 \
shared-mime-info-1.8-4.el7.x86_64 \
sharp-1.8.1.MLNX20190422.6c05a05-1.46101.x86_64 \
shim-x64-15-2.el7.centos.x86_64 \
slang-2.2.4-11.el7.x86_64 \
smartmontools-6.5-1.el7.x86_64 \
snappy-1.1.0-3.el7.x86_64 \
sos-3.6-19.el7.centos.noarch \
sqlite-3.7.17-8.el7.x86_64 \
srptools-41mlnx1-5.46101.x86_64 \
ssacli-3.40-3.0.x86_64 \
sssd-client-1.16.2-13.el7_6.8.x86_64 \
stix-fonts-1.1.0-5.el7.noarch \
strace-4.12-9.el7.x86_64 \
sudo-1.8.23-3.el7.x86_64 \
sysfsutils-2.1.0-16.el7.x86_64 \
syslinux-4.05-15.el7.x86_64 \
sysstat-10.1.5-17.el7_6.1.x86_64 \
systemd-219-62.el7_6.9.x86_64 \
systemd-libs-219-62.el7_6.9.x86_64 \
systemd-python-219-62.el7_6.9.x86_64 \
systemd-sysv-219-62.el7_6.9.x86_64 \
systemtap-runtime-3.3-3.el7.x86_64 \
systemtap-sdt-devel-3.3-3.el7.x86_64 \
sysvinit-tools-2.88-14.dsf.el7.x86_64 \
tar-1.26-35.el7.x86_64 \
targetcli-2.1.fb46-7.el7.noarch \
tcl-8.5.13-8.el7.x86_64 \
tcpdump-4.9.2-3.el7.x86_64 \
tcp_wrappers-7.6-77.el7.x86_64 \
tcp_wrappers-libs-7.6-77.el7.x86_64 \
tcsh-6.18.01-15.el7.x86_64 \
teamd-1.27-6.el7_6.1.x86_64 \
tftp-5.2-22.el7.x86_64 \
time-1.7-45.el7.x86_64 \
tk-8.5.13-6.el7.x86_64 \
tmpwatch-2.11-5.el7.x86_64 \
trace-cmd-2.6.0-10.el7.x86_64 \
traceroute-2.0.22-2.el7.x86_64 \
trousers-0.3.14-2.el7.x86_64 \
ttmkfdir-3.0.9-42.el7.x86_64 \
tuned-2.10.0-6.el7_6.4.noarch \
tzdata-2019b-1.el7.noarch \
tzdata-java-2019b-1.el7.noarch \
ucx-1.6.0-1.46101.x86_64 \
ucx-cma-1.6.0-1.46101.x86_64 \
ucx-devel-1.6.0-1.46101.x86_64 \
ucx-ib-1.6.0-1.46101.x86_64 \
ucx-ib-cm-1.6.0-1.46101.x86_64 \
ucx-knem-1.6.0-1.46101.x86_64 \
ucx-rdmacm-1.6.0-1.46101.x86_64 \
ucx-static-1.6.0-1.46101.x86_64 \
unixODBC-2.3.1-11.el7.x86_64 \
unzip-6.0-19.el7.x86_64 \
urw-fonts-2.4-16.el7.noarch \
usb_modeswitch-2.5.1-1.el7.x86_64 \
usb_modeswitch-data-20170806-1.el7.noarch \
usbutils-007-5.el7.x86_64 \
usermode-1.111-5.el7.x86_64 \
ustr-1.0.4-16.el7.x86_64 \
util-linux-2.23.2-59.el7_6.1.x86_64 \
uuid-1.6.2-26.el7.x86_64 \
valgrind-3.13.0-13.el7.x86_64 \
vim-common-7.4.160-6.el7_6.x86_64 \
vim-enhanced-7.4.160-6.el7_6.x86_64 \
vim-filesystem-7.4.160-6.el7_6.x86_64 \
vim-minimal-7.4.160-6.el7_6.x86_64 \
virt-what-1.18-4.el7.x86_64 \
vulkan-filesystem-1.1.73.0-1.el7.noarch \
wget-1.14-18.el7_6.1.x86_64 \
wg-gmetric-12.0-5.noarch \
wg-hostfunc-1.3-1.noarch \
wg-nfsstale-12.0.0-0.noarch \
wg-o2dk-11.3.7-18.el5.x86_64 \
which-2.20-7.el7.x86_64 \
wireshark-1.10.14-16.el7.x86_64 \
words-3.0-22.el7.noarch \
xcb-util-0.4.0-2.el7.x86_64 \
xdg-utils-1.1.0-0.17.20120809git.el7.noarch \
xfsdump-3.1.7-1.el7.x86_64 \
xfsprogs-4.5.0-19.el7_6.x86_64 \
xkeyboard-config-2.24-1.el7.noarch \
xmlrpc-c-1.32.5-1905.svn2451.el7.x86_64 \
xmlrpc-c-client-1.32.5-1905.svn2451.el7.x86_64 \
xorg-x11-drivers-7.7-6.el7.x86_64 \
xorg-x11-drv-ati-18.1.0-1.el7_6.x86_64 \
xorg-x11-drv-dummy-0.3.7-1.el7.1.x86_64 \
xorg-x11-drv-evdev-2.10.6-1.el7.x86_64 \
xorg-x11-drv-fbdev-0.5.0-1.el7.x86_64 \
xorg-x11-drv-intel-2.99.917-28.20180530.el7.x86_64 \
xorg-x11-drv-nouveau-1.0.15-1.el7.x86_64 \
xorg-x11-drv-qxl-0.1.5-4.el7.1.x86_64 \
xorg-x11-drv-synaptics-1.9.0-2.el7.x86_64 \
xorg-x11-drv-v4l-0.2.0-49.el7.x86_64 \
xorg-x11-drv-vesa-2.4.0-1.el7.x86_64 \
xorg-x11-drv-vmmouse-13.1.0-1.el7.1.x86_64 \
xorg-x11-drv-vmware-13.2.1-1.el7.1.x86_64 \
xorg-x11-drv-void-1.4.1-2.el7.1.x86_64 \
xorg-x11-drv-wacom-0.36.1-1.el7.x86_64 \
xorg-x11-fonts-Type1-7.5-9.el7.noarch \
xorg-x11-font-utils-7.5-21.el7.x86_64 \
xorg-x11-server-common-1.20.1-5.6.el7_6.x86_64 \
xorg-x11-server-utils-7.7-20.el7.x86_64 \
xorg-x11-server-Xorg-1.20.1-5.6.el7_6.x86_64 \
xorg-x11-utils-7.5-23.el7.x86_64 \
xorg-x11-xauth-1.0.9-1.el7.x86_64 \
xorg-x11-xbitmaps-1.1.1-6.el7.noarch \
xorg-x11-xinit-1.3.4-2.el7.x86_64 \
xorg-x11-xkb-utils-7.7-14.el7.x86_64 \
xterm-295-3.el7.x86_64 \
xz-5.2.2-1.el7.x86_64 \
xz-libs-5.2.2-1.el7.i686 \
xz-libs-5.2.2-1.el7.x86_64 \
yajl-2.0.4-4.el7.x86_64 \
ypbind-1.37.1-9.el7.x86_64 \
yp-tools-2.14-5.el7.x86_64 \
yum-3.4.3-161.el7.centos.noarch \
yum-metadata-parser-1.1.4-10.el7.x86_64 \
yum-plugin-fastestmirror-1.1.31-50.el7.noarch \
yum-plugin-nvidia-0.3-1.el7.noarch \
yum-utils-1.1.31-50.el7.noarch \
zip-3.0-11.el7.x86_64 \
zlib-1.2.7-18.el7.i686 \
zlib-1.2.7-18.el7.x86_64 \
zlib-devel-1.2.7-18.el7.x86_64